...
public class WireFrame {
    ...
    public WireFrame(String FileName) {
        ...
        scan.useDelimiter("\n");
        int num = Integer.decode(scan.next().trim());//the number of points
        points = new ThreeDPoint[num];
        for (int i = 0; i < num; i++) {
            String sPoint = (scan.next());//a line in the file for a point
            Scanner scan2 = new Scanner(sPoint);
            scan2.useDelimiter(",");
            points[i] = new ThreeDPoint(//breaks the line into the x,y,z
                    Integer.decode(scan2.next().trim()),
                    Integer.decode(scan2.next().trim()),
                    Integer.decode(scan2.next().trim())
            );
        }
        faceNum = Integer.decode(scan.next().trim());//the number of faces
        ...
        for (int face = 0; face < faceNum; face++) {
            ...
            scan2.useDelimiter(",");
            for (int i = 0; i < 3; i++) {
                faces[face][i] = Integer.decode(scan2.next().trim());
            }
            RGB[face] = Integer.decode(scan2.next().trim());//the hexidecimal
        }
    }
}
